...
(function () {
  ...
  */
  ...
  /* Patch "Beyond thumbnails." section header */
  function patchBeyondThumbnails() {
    document.querySelectorAll('*').forEach(function(el) {
      /* Hide the description */
      if (el.children.length === 0 && (el.textContent || '').trim() === 'Long-form, motion, and everything that moves frame to frame.') {
        el.style.setProperty('display', 'none', 'important');
        /* Center the heading row (parent of this p and the h2) */
        var row = el.parentElement;
        if (row) {
  ...
  /* Patch CTA headline: "Let's build something clickable." → "Are you ready?" */
  function patchCTAHeadline() {
    document.querySelectorAll('*').forEach(function(el) {
      if (el.children.length === 0 && (el.textContent || '').trim() === 'clickable.') {
        el.textContent = 'ready?';
        var parent = el.parentElement;
        if (parent) {
          /* Replace the preceding text node */
          parent.childNodes.forEach(function(node) {
            if (node.nodeType === 3 && node.textContent.includes("Let's build")) {
              node.textContent = 'Are you ';
            }
          });
          var P = 'important';
          parent.style.setProperty('font-size', 'clamp(2.75rem, 6.5vw, 5rem)', P);
          parent.style.setProperty('white-space', 'normal', P);
          parent.style.setProperty('padding-bottom', '0.15em', P);
          parent.style.setProperty('padding-right', '0.3em', P);
          parent.style.setProperty('line-height', '1.1', P);
        }
      }
    });
  }
